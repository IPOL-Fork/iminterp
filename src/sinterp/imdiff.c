 ...
 * read 16-bit PNG images (provided USE_LIBPNG is enabled), the image data
...
{
    puts("Image difference calculator, P. Getreuer 2010-2011\n");
    puts("Usage: imintdiff [options] <exact file> <distorted file>\n"
        "Only " READIMAGE_FORMATS_SUPPORTED " images are supported.\n");
    puts("Options:");
    puts("   -m <metric>  Metric to use for comparison, choices are");
    puts("        max     Maximum absolute difference, max_n |A_n - B_n|");
    ...
    puts("        mssim   Mean structural similarity index\n");
    ...
    "   imintdiff [-D <number>] <exact file> <distorted file> <output file>\n");
    puts("The difference image is computed as\n"
           "   D_n = 255/D (A_n - B_n) + 255/2.\n"
           "Values outside of the range [0,255] are saturated.\n");
    puts("Example:\n"
#ifdef USE_LIBPNG
        "   imintdiff -mpsnr frog-exact.png frog-4x.png");
#else
        "   imintdiff -mpsnr frog-exact.bmp frog-4x.bmp");
#endif
}

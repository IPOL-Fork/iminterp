...
int main(int argc, char *argv[])
{
    ...
    if(!(A.data = (float *)ReadImage(&A.width, &A.height, param.file_a,
        IMAGEIO_FLOAT | IMAGEIO_RGB | IMAGEIO_PLANAR)))
        goto fail;
    
    /* Read the distorted image. */
    if(!(B.data = (float *)ReadImage(&B.width, &B.height, param.file_b,
        IMAGEIO_FLOAT | IMAGEIO_RGB | IMAGEIO_PLANAR)))
        goto fail;
    ...
    if(param.difference_file)
    {
        ...
        if(!(WriteImage(A.data, A.width, A.height, param.difference_file,

...
{
    puts("Image mosaicing utility, P. Getreuer 2010-2011\n");
    puts("Usage: imintmosaic [options] <input file> <output file>\n\n"
        "Only " READIMAGE_FORMATS_SUPPORTED " images are supported.\n");
    puts("Options:");
    puts("  -p <pattern>  CFA pattern, choices for <pattern> are");
    puts("                RGGB        upperleftmost red pixel is at (0,0)");
    puts("                GRBG        upperleftmost red pixel is at (1,0)");
    puts("                GBRG        upperleftmost red pixel is at (0,1)");
    puts("                BGGR        upperleftmost red pixel is at (1,1)\n");
    puts("  -f            Flatten result to a grayscale image");
    puts("  -r            Add one extra row to the top");
    puts("  -c            Add one extra column to the left");
    puts("  -e <padding>  Add <padding> pixels to each border of the image");
    puts("  -v            Verbose output\n");
#ifdef USE_LIBJPEG
    puts("  -q <number>   Quality for saving JPEG images (0 to 100)\n");
#endif
    puts("Example: \n"
         "  imintmosaic -v -p RGGB frog.bmp frog-m.bmp");
}
